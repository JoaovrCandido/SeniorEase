...
export default function NotebookEditorPage() {
  ...
  const executeNotebookDelete = async () => {
    ...
    showToast(
      `${isTodoList ? "Lista movida" : "Caderno movido"} para a lixeira.`,
      "info",
    );
    router.push("/");
  };
  ...
  return (
    <main className={styles.main}>
          ...
          <section>
            {activeBlocks.length === 0 ? (
              <p className={styles.emptyState}>
                A sua {isTodoList ? "lista" : "caderno"} está{" "}
                {isTodoList ? "vazia" : "vazio"}. Comece a adicionar conteúdo
                abaixo!
              </p>
            ) : (
              activeBlocks.map((block, index) => {
                    ...
                    case "heading":
                      return (
                        <HeadingBlockUI
                          ...
                          onMoveUp={() =>
                            moveBlock(activeNotebook.id, block.id, "up")
                          }
                          onMoveDown={() =>
                            moveBlock(activeNotebook.id, block.id, "down")
                          }
                          onDelete={(id) =>
                            actionConfirmation === "on"
                              ? setBlockToDelete(id)
                        ...
                        <TaskBlockUI
                          ...
                          onToggle={async (id) => {
                            ...
                            if (!block.isCompleted)
                              showToast(
                                "Parabéns concluído com sucesso!",
                                "success",
                              );
                          }}
                          onChangeContent={(id, content) =>
                ...
                return (
                  <div
                    key={block.id}
                    className="advanced-block-container"
      ...
      <Modal
        ...
        title={`Editar ${isTodoList ? "Lista" : "Caderno"}`}
      >
      ...
      <Modal
        ...
        title={`Apagar ${isTodoList ? "Lista" : "Caderno"}`}
      >
        <div className={styles.modalContent}>
          <p className={styles.textBaseHeading}>
            Tem certeza que deseja apagar{" "}
            <strong>{activeNotebook.title}</strong>? O ficheiro será enviado
            para a lixeira.
          </p>
          <div className={styles.modalFooter}>
