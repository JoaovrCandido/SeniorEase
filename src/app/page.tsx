...
export default function Home() {
  const { 
    notebooks, isLoading, createNotebook, addBlock, toggleTask, 
    updateBlock, deleteBlock, updateNotebookInfo, deleteNotebook 
  } = useNotebook();
  ...
  if (activeNotebookId && activeNotebook) {
    ...
    const compiledTextToRead = activeNotebook.blocks.map(b => {
      ...
      <main className={styles.main}>
        <div className={styles.editor}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {!isReadingMode && (
                <div className={styles.headerActions}>
                  <button className={styles.secondaryButton} onClick={openEditModal}>✏️ Editar Nome e Descrição</button>
          ...
          {isReadingMode ? (
            <article className={styles.readingContainer}>
              {activeNotebook.blocks.length === 0 ? <p className={styles.readParagraph}>Este caderno está vazio.</p> : activeNotebook.blocks.map((block) => {
                  switch (block.type) {
            ...
            <section>
              {activeNotebook.blocks.length === 0 ? <p className={styles.emptyState}>O seu caderno está vazio. Comece a adicionar conteúdo na barra abaixo!</p> : activeNotebook.blocks.map((block) => {
                  switch (block.type) {
                    case 'heading': return <HeadingBlockUI key={block.id} block={block} onDelete={(id) => deleteBlock(activeNotebook.id, id)} onChangeContent={(id, content) => updateBlock(activeNotebook.id, id, { content })} />;
                    case 'task': return <TaskBlockUI key={block.id} block={block} onToggle={(id) => toggleTask(activeNotebook.id, id)} onDelete={(id) => deleteBlock(activeNotebook.id, id)} onChangeContent={(id, content) => updateBlock(activeNotebook.id, id, { content })} />;
                    case 'paragraph': return <ParagraphBlockUI key={block.id} block={block} onDelete={(id) => deleteBlock(activeNotebook.id, id)} onChangeContent={(id, content) => updateBlock(activeNotebook.id, id, { content })} />;
                    case 'meeting': return <MeetingBlockUI key={block.id} block={block} onDelete={(id) => deleteBlock(activeNotebook.id, id)} onChangeContent={(id, title, meetingUrl, date) => updateBlock(activeNotebook.id, id, { title, meetingUrl, date })} />;
                    case 'reminder': return <ReminderBlockUI key={block.id} block={block} onDelete={(id) => deleteBlock(activeNotebook.id, id)} onChangeContent={(id, content, date) => updateBlock(activeNotebook.id, id, { content, date })} />;
                    default: return null;
                  }
              })}
            </section>
        ...
        {!isReadingMode && (
          <nav className={styles.bottomBar} aria-label="Adicionar elementos" style={{ flexWrap: 'wrap' }}>
            {!hasHeading && <button className={styles.actionButton} onClick={() => addBlock(activeNotebook.id, { type: 'heading', content: '' })}><span aria-hidden="true" style={{ fontSize: '24px' }}>📝</span> Título</button>}
            <button className={styles.actionButton} onClick={() => addBlock(activeNotebook.id, { type: 'paragraph', content: '' })}><span aria-hidden="true" style={{ fontSize: '24px' }}>✍️</span> Anotação</button>
            <button className={styles.actionButton} onClick={() => addBlock(activeNotebook.id, { type: 'task', content: '', isCompleted: false })}><span aria-hidden="true" style={{ fontSize: '24px' }}>✅</span> Tarefa</button>
            <button className={styles.actionButton} onClick={() => addBlock(activeNotebook.id, { type: 'reminder', content: '', date: '' })}><span aria-hidden="true" style={{ fontSize: '24px' }}>⏰</span> Lembrete</button>
            <button className={styles.actionButton} onClick={() => addBlock(activeNotebook.id, { type: 'meeting', title: '', meetingUrl: '', date: '' })}><span aria-hidden="true" style={{ fontSize: '24px' }}>🎥</span> Reunião</button>
          </nav>
        )}
        ...
        <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Apagar Caderno">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
            <p style={{ fontSize: 'var(--text-base)', color: 'var(--text-heading)' }}>
              Tem certeza que deseja apagar o caderno <strong>{activeNotebook.title}</strong>? Esta ação não pode ser desfeita e todo o conteúdo será perdido.
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-md)' }}>
