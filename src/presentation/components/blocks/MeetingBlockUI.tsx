...
export const MeetingBlockUI: React.FC<Props> = ({ block, onChangeContent, onDelete }) => {
  ...
  const handleBlur = () => {
    if (title !== block.title || url !== block.meetingUrl || date !== block.date) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px', padding: '16px', backgroundColor: '#EFF6FF', borderRadius: 'var(--radius-md)', border: '2px solid #BFDBFE' }}>
      <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1, gap: '8px' }}>
          <input 
            type="text" value={title} onChange={(e) => setTitle(e.target.value)} onBlur={handleBlur}
          ...
          />
          {/* NOVO CAMPO: Horário da Reunião */}
          <input 
            type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} onBlur={handleBlur}
            style={{ height: '48px', border: 'none', background: 'transparent', fontSize: 'var(--text-base)', color: 'var(--text-heading)' }}
          />
          <input 
            type="url" value={url} onChange={(e) => setUrl(e.target.value)} onBlur={handleBlur}
            placeholder="Cole o link (Zoom, Meet, Teams...)"
            style={{ height: '48px', border: 'none', background: 'transparent', fontSize: 'var(--text-base)', color: 'var(--text-body)' }}
          />
        </div>
      ...
      {url && (
        <a href={url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
          <Button variant="primary" style={{ width: '100%', marginTop: '8px' }}>Entrar na Reunião</Button>
        </a>
      )}
    </div>
  );
