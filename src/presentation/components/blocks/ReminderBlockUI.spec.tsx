...
describe('ReminderBlockUI', () => {
  ...
  it('deve mostrar erro e NÃO gravar se o utilizador escolher uma data no passado', () => {
    ...
    render(<ReminderBlockUI block={mockBlock} onDelete={vi.fn()} onChangeContent={mockOnChange} />);
    ...
    // Valida o bloqueio!
    expect(mockShowToast).toHaveBeenCalledWith("O lembrete não pode ser marcado no passado.", "error");
  ...
  it('deve gravar corretamente se a data for no futuro', () => {
    ...
    expect(mockOnChange).toHaveBeenCalledWith('r-1', 'Tomar remédio', '2026-07-27T10:00');
  });
});