...
const TestComponent = () => {
  const { name, age, emergencyContact, setName, setAge, setEmergencyContact } = useUserProfile();
  ...
  return (
    <div>
      ...
      <button onClick={() => setEmergencyContact('912345678')}>Mudar Contato</button>
    </div>
  );
};
...
describe('UserProfileContext', () => {
  ...
  it('deve carregar os dados da localStorage ao montar (simulando que o utilizador já lá tinha estado)', () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({ name: 'Maria José', age: '80', emergencyContact: '112' })
    );
